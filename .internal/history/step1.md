...
## Q1
아래 내용에 대한 프롬프트를 만들어주세요.


목표: lean-dive 라는 이름의 gemini cli 스킬들을 만드는 것이 목표이며, 세부 하위 스킬들은 다음과 같음

lean-dive
* lean-dive:hello
* lean-dive:official-docs
* lean-dive:question
* lean-dive:grouping
* lean-dive:documenting


'lean-dive' 라는 이름으로 하위 gemini skill 들을 만들려고 하는데 다음과 같음

스킬 상세
* lean-dive:hello (requirement 수집, 요구사항 파악)
  * 사용자에게 무엇을 원하는지, 어떤 주제인지를 전달받음
* lean-dive:official-docs (공식문서 수집)
  * 사용자가 원하는 주제에 대해 찾기 원하는 공식문서들를 찾음
  * 또는 사용자에게 받은 공식문서 링크를 갈무리
* lean-dive:question
  * 사용자가 하는 질문들에 대한 답변을 원하는 만큼 수행 후 질문/답변 내용을 정리 
  * e.g. .internal-work/6.6-questions.md
  * 루프의 마지막에 사용자에게 파일명을 물어보고, 별도의 요구가 없다면 파일명은 QUESTIONS.md 라는 이름으로 저장, 만약 QUESTIONS.md 라는 파일이 있다면 QUSTIONS1.md, QUSTIONS1.md 파일이 이미 있다면 QUSTIONS2.md 등이 되도록 파일명을 지정
* lean-dive:grouping
  * 특정 주제 단위로 나눌지 or 하나의 파일에 모두 정리할지를 물어보는 단계
  * 사용자에게 방금 생성한 QUESTION 파일에 대해 어떤 의미 단위로 그루핑할지를 문의
  * 그루핑을 하실겁니까? 등으로 물어보고 NO/No/N/n 으로 답변할 경우 하나의 파일에 모두 정리, Yes/YES/Y/y 로 답변할 경우 그루핑 단위를 문의
* lean-dive:documenting
  * 지금까지의 결과를 바탕으로 정리한 자료들을 분류 및 정리


스킬 상세를 랄프루프로 돌리는 경우 Antigravity 에 입력할 프롬프트를 제안해줘

참고)

단계 설명
hello -> [official-docs|question] -> (optional)grouping -> (final) documenting

각 단계는 사용자의 호출에 의해 시작되며 다음 단계는 무엇인지, 현재 단계에서 뭘 선택할수 있는지를 안내해야 함
e.g.
...
... 다음 단계로 넘어가시겠습니까? (y/n)
... y 일 경우 lean-dive:official-docs, lean-dive:question 중 무엇을 선택할지 질문
... n 일 경우 계속 요구사항 수집


/lean-dive:[official-docs|question]

... 다음 단계로 넘어가시겠습니까? (y/n)
... y 일 경우 lean-dive:grouping 을 할지, lean-dive:documenting 을 할지 질문
... n 일 경우 다음 턴으로
...
* 특정 주제 단위로 나눌지 or 하나의 파일에 모두 정리할지를 물어보는 단계
  * 사용자에게 방금 생성한 QUESTION 파일에 대해 어떤 의미 단위로 그루핑할지를 문의
  * 그루핑을 하실겁니까? 등으로 물어보고 NO/No/N/n 으로 답변할 경우 하나의 파일에 모두 정리, Yes/YES/Y/y 로 답변할 경우 그루핑 단위를 문의 후 그루핑 단위를 사용자에게 입력받음
  * 사용자가 입력한 그루핑 단위를 기억하고 documenting 으로 넘어감
...
## 답변
어서 오세요! `lean-dive` 스킬 시리즈를 위해, Antigravity(또는 Gemini CLI Agent)가 **Ralph Loop**의 사상에 맞춰 자율적으로 단계를 제안하고 실행할 수 있도록 최적화된 시스템 프롬프트 세트를 제안해 드립니다.

이 프롬프트들은 각 단계의 **수행 목표**, **기술적 로직(파일명 처리 등)**, 그리고 **다음 단계 안내(Next Action Guide)**를 포함하고 있어, 사용자가 흐름을 잃지 않고 프로젝트를 완수할 수 있도록 돕습니다.
